...
class geo {
    ...
    private $start = false;
    public function start ($db_file = false, $type = SXGEO_FILE){

        if( $this->start === true )
            return true;

        if( !$db_file )
            $db_file = $_SERVER['DOCUMENT_ROOT'].'/backend/components/plugins/geo/SxGeoCity.dat';
    ...
    public function get_num($ip=false){
        $this->start();
    ...
    public function getCountryId($ip=false){
        $this->start();
    ...
    public function getCity($ip=false){
        $db = db::getInstance();
        ...
        $info = $seek ? $this->parseCity($seek) : false;

        $data = $db->getRow('SELECT * FROM `geo_city` WHERE `name`=?s LIMIT 1',$info['city']['name_ru']);

        if( !$data )
            $data = $db->getRow('SELECT * FROM `geo_city` WHERE `name`=?s','Москва');
    ...
    public function getCityId($city_id){
        ...
        $data = $db->getRow('SELECT * FROM `geo_city` WHERE `id`=?i', (int)$city_id);
        return $data;

    }

    public function getCityFull($ip=false){
        $this->start();
    ...
    public $allCity = false;
    public  function getAllCity($lang,$name=false){
        $db = db::getInstance();

        if(!$lang) $lang='ru';
        
        if( $name ){
            $rows = $db->getAll('SELECT c.id,cl.title FROM `geo_city` c INNER JOIN geo_city_lang cl ON (c.id = cl.city_id AND cl.lang=?s) WHERE cl.`title` LIKE ?s LIMIT 20',$lang,'%'.$name.'%');
        }else{

/*            if( $this->allCity )
                return $this->allCity;*/

            $rows = $db->getAll('SELECT c.id,cl.title FROM `geo_city` c INNER JOIN geo_city_lang cl ON (c.id = cl.city_id AND cl.lang=?s) LIMIT 20',$lang);

            // c LEFT JOIN geo_city_lang cl ON (c.id = cl.city_id AND cl.lang=?s)
           /* $rows = $db->getAll('SELECT c.id,cl.title FROM `geo_city` c INNER JOIN geo_city_lang cl ON (c.id = cl.city_id AND cl.lang=?s) WHERE c.`name` IN (
              "Москва",
              "Санкт-Петербург",
              "Новосибирск",
              "Екатеринбург",
              ...
              "Воронеж",
              "Волгоград"
            ) ORDER BY c.`name` ASC',$lang);*/
            //Получаем те горрода где есть рестораны
            //$rows = $db->getAll('SELECT * FROM `geo_city` WHERE `id` IN ( SELECT `city_id` FROM `restaurants` ) ORDER BY `name` ASC LIMIT 15');
        }


        $return = array();
        foreach( $rows as $row ){
            $return[] = array('id'=>$row['id'],'text'=>$row['title']);
        }
    ...
    //Вытащить все районы с города
    public  function getDistricts($city_id,$json=true)
    {
        $db = db::getInstance();

        $city_id = (int)$city_id;
        $districts = $db->getAll('SELECT * FROM `geo_districts` WHERE `city_id`=?i ',$city_id);
