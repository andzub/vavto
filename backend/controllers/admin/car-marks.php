...
//echo "<pre>".print_r($_SERVER,true)."</pre>";echo "<pre>".print_r($config,true)."</pre>";die();
    ...
    if( $this->GET[0] == 'add' ){
        ...
        if( $_POST['ajax'] ){
            ...
            $response = $api->post('/car-marks/',$_POST);
            echo json_encode($response);
            exit;

        }

        $template = 'car-marks-add.html';
    ...
    if( $this->GET[0] == 'edit' ){
        ...
        if( $_POST['ajax'] ){
            ...
            $_POST['img'] = $_FILES['img'];
            $_POST['img_min'] = $_FILES['img_min'];
            $response = $api->put('/car-marks/',$_POST);
            echo json_encode($response);
            exit;

        }

        $row = $api->get('/car-marks/', $this->GET[1]);
        if( !$row ){
            header('Location: /admin/car-marks/');
            exit;
        }

        $template = 'car-marks-edit.html';

    }

    if( $this->GET[0] == 'delete' ){
        $api->delete('/car-marks/',$this->GET[1]);
        header('Location: /admin/car-marks/');
        exit;
    }
