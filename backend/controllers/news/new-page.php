...
$url=$url_parts[0];
$page = $db->getRow('SELECT a.*,al.title,al.text,al.short_desc,al.lang FROM `articles` a INNER JOIN articles_lang al ON (a.id=al.article_id AND al.lang="'.$config['lang'].'") WHERE `address`=?s AND type=2',$url);
if( $page ){
