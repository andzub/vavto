<?php
    ...
    $vars['workCategory']=$settings->workMainCategory($config['lang']); //$work_cats
    //$vars['workCategory2']=$settings->GetAllCategorys($config['lang']); //$work_cats
...
//exit;
    ...
    for($i=0;$i<$w_count;$i++){
	    ...
	    $coll_arr[$work_cats[$i]['id']]=$work_cats[$i];
	...
	/*foreach($work_cats as $key)
        $vars['workCategory'][$key['id']]=$key;*/
    //  $vars['workCategory']=$work_cats;
   ...
   /* echo "++++++++++++++++++++++<pre>";
    ...
    //var_dump($workType);
	/*foreach($workType as $key)
        $vars['workType'][$key['cat_id']]=$key;*/
    ...
    function send($data,$vars){
		...
		if(isset($data['brand']) || $data['brand'])
        {
            if($data['model_'.$brand['id']] && isset($vars['models'][$brand['id']][(int)$data['model_'.$brand['id']]]))
            {
                $model=$vars['models'][$brand['id']][(int)$data['model_'.$brand['id']]];
            }
        }
        ...
        if(!$category_name) $category_name='Другое';
        if(!$model['id']) $model['name']='Не указана';
...
if($email)
{
    ...
    if(!$is_set_user) // если нет  - регистрируем нового клиента
    {
        ...
        <a href='".$config['domain']."/cabinet/login/'>Вход в личный кабинет по ссылке</a>";
    }
}
